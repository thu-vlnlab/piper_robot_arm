"""
方案2: 平滑过渡 (Smooth Blending)
每次收到新buffer时，与上一次执行的action做平滑插值过渡，避免跳变
"""
import dataclasses
import enum
import logging
...
class Args:
    ...
    # 平滑参数
    blend_steps: int = 5  # 平滑过渡的步数
    blend_method: str = "linear"  # 平滑方法: "linear" 或 "exponential"
...
def _plot_actions(actions: np.ndarray, save_path: pathlib.Path, timestamp: str) -> None:
    ...
    fig.suptitle(f'[SMOOTH] Executed Action Trajectories (Total Steps: {num_steps})\n{timestamp}', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plot_path = save_path / f"plot1_executed_actions_smooth_{timestamp}.png"
...
def _plot_comparison(executed_actions: np.ndarray, actual_positions: np.ndarray, save_path: pathlib.Path, timestamp: str) -> None:
    ...
    axes[-1].set_xlabel('Step', fontsize=11)
    fig.suptitle(f'[SMOOTH] Command vs Actual Position Comparison (Total Steps: {num_steps})\n{timestamp}', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plot_path = save_path / f"plot2_comparison_smooth_{timestamp}.png"
    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
...
def _plot_request_actions(request_records: list, save_path: pathlib.Path, timestamp: str) -> None:
    ...
    fig.suptitle(f'[SMOOTH] Request Action Sequences (Total Requests: {len(request_records)})\n{timestamp}', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.subplots_adjust(bottom=0.05)
    plot_path = save_path / f"plot3_request_actions_smooth_{timestamp}.png"
    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    logger.info(f"✅ 图3 已保存: {plot_path}")


def _blend_action(last_action: np.ndarray, target_action: np.ndarray,
                  blend_step: int, total_blend_steps: int, method: str = "linear") -> np.ndarray:
    """
    平滑混合两个动作

    Args:
        last_action: 上一个执行的动作
        target_action: 目标动作（新buffer中的）
        blend_step: 当前是第几步混合（0-based）
        total_blend_steps: 总共需要多少步完成混合
        method: 混合方法 "linear" 或 "exponential"

    Returns:
        混合后的动作
    """
    if blend_step >= total_blend_steps:
        return target_action.copy()

    if method == "linear":
        # 线性插值: alpha 从 0 逐渐增加到 1
        alpha = (blend_step + 1) / total_blend_steps
    elif method == "exponential":
        # 指数平滑: 更快地接近目标
        alpha = 1 - np.exp(-3 * (blend_step + 1) / total_blend_steps)
    else:
        alpha = (blend_step + 1) / total_blend_steps

    blended = (1 - alpha) * last_action + alpha * target_action
    return blended
...
def main(args: Args) -> None:
    ...
    logger.info(f"🔧 方案: 平滑过渡 (Smooth Blending)")
    logger.info(f"🔧 平滑参数: blend_steps={args.blend_steps}, method={args.blend_method}")
    ...
    # 【方案2核心】平滑过渡相关变量
    last_executed_action = None  # 上一个执行的动作
    blend_remaining = 0  # 剩余需要平滑的步数
    blend_start_action = None  # 平滑开始时的动作
    ...
    with tqdm.tqdm(total=args.num_steps, desc="Executing actions [SMOOTH]") as pbar:
        while total_executed < args.num_steps:
            ...
            with request_lock:
                if next_action_buffer is not None:
                    ...
                    if last_executed_action is not None:
                        blend_remaining = args.blend_steps
                        blend_start_action = last_executed_action.copy()
                        logger.info(f"🔧 [平滑过渡] 开始{args.blend_steps}步平滑，从上一动作过渡到新轨迹")
            ...
            # 【方案2核心】应用平滑过渡
            if blend_remaining > 0 and blend_start_action is not None:
                blend_step = args.blend_steps - blend_remaining
                current_action = _blend_action(
                    blend_start_action, raw_action,
                    blend_step, args.blend_steps,
                    args.blend_method
                )
                blend_remaining -= 1
                if blend_remaining == 0:
                    logger.info(f"🔧 [平滑过渡] 平滑过渡完成")
            else:
            ...
            if total_executed % 5 == 1 or total_executed <= 3:
                blend_info = f" [BLEND {args.blend_steps - blend_remaining}/{args.blend_steps}]" if blend_remaining > 0 else ""
                action_str = ', '.join([f"{val:+.6f}" for val in current_action])
                logger.info(f"  Step {total_executed:3d} | Buffer[{action_index-1}]{blend_info} | Action: [{action_str}]")
    ...
    if len(executed_actions_list) > 0:
        import pickle
        pkl_path = args.output_dir / f"actions_output_smooth_{timestamp}.pkl"
        save_data = {
            'executed_actions': executed_actions_list,
            'actual_positions': actual_positions_list,
            'request_records': request_records,
            'timestamp': timestamp,
            'method': 'smooth_blending',
            'blend_params': {
...
def _init_ros_node():
    ...
    class ROSDataCollector(Node):
        def __init__(self):
            super().__init__('openpi_data_collector_smooth')
            ...
            self.get_logger().info("ROS节点已初始化 [SMOOTH]，等待数据...")
