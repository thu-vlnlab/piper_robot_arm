...
def _interpolate_actions(actions: np.ndarray, source_freq: float = 15.0, target_freq: float = 45.0) -> np.ndarray:
    """
    使用三次样条插值将action序列从source_freq插值到target_freq
    ...
    for joint_idx in range(num_joints):
        cs = CubicSpline(source_times, actions[:, joint_idx])
        interpolated_actions[:, joint_idx] = cs(target_times)

    logger.info(f"🔄 [插值] {source_freq}Hz ({num_actions}步) -> {target_freq}Hz ({num_target_actions}步)")
...
def main(args: Args) -> None:
    ...
    def async_request_worker(request_id):
        ...
        try:
            ...
            if 'actions' in action_response:
                ...
                logger.info(f"✅ [请求 {request_id}] 后台接收原始序列: shape={raw_actions.shape} (15Hz), 耗时={inference_time_ms:.1f}ms")

                # 使用三次样条插值从15Hz插值到45Hz
                interpolated_actions = _interpolate_actions(raw_actions, source_freq=15.0, target_freq=45.0)
                ...
                logger.info(f"✅ [请求 {request_id}] 插值后序列: shape={next_action_buffer['actions'].shape} (45Hz)")
                # === 新代码结束 ===
            else:
    ...
    if 'actions' in first_response:
        ...
        logger.info(f"✅ [初始化] 收到原始序列: shape={raw_actions.shape} (15Hz)")

        # 使用三次样条插值从15Hz插值到45Hz
        action_buffer = _interpolate_actions(raw_actions, source_freq=15.0, target_freq=45.0)
        action_index = 0
        ...
        logger.info(f"✅ [初始化] 插值后序列: shape={action_buffer.shape} (45Hz)")
        # === 新代码结束 ===
        ...
        while total_executed < args.num_steps:
            ...
            with request_lock:
                if next_action_buffer is not None:
                    ...
                    # 【方案1核心】计算新的偏移量：当前实际位置 - 新buffer起始位置
                    if latest_joint_state is not None:
                        new_start_pos = action_buffer[action_index]
                        current_offset = latest_joint_state - new_start_pos
